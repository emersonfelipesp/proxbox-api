...
3. **Data-only request** — `.gitea/workflows/publish-gitea.yml` first requires the exact successful GitHub-hosted offline-image job for the same canonical `develop` SHA and verifies the source-SHA GitHub workflow bytes against the reviewed SHA-256 before trusting that job. The offline job pins every external action by immutable commit. It then builds and uploads the exact signed six-file request: wheel, sdist, manifest, canonical request, supervisor completion statement, and detached signature. That statement binds the supervisor-derived repository-registration scope digest, and the target client requires it to match the pinned acceptance record. The workflow has no package or mirror credential and cannot publish or push tags.
