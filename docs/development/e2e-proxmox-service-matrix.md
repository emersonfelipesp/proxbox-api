...
The CI matrix is generated by the tested `scripts/ci_e2e_matrix.py` helper. It
always emits every transport, supported NetBox version, selected package mode,
and PVE path. Push/manual runs also emit the full PBS/PDM cross-product. A
release doubles the package modes, so PBS/PDM run on the baseline transport
while PVE retains all seven transports; this keeps release coverage within
GitHub's hard 256-job matrix limit.

| Axis | Source | Default |
|---|---|---|
| `base` (transport bundle) | `scripts/ci_e2e_matrix.py` | 7 transport combinations covering `http_manage`, `https_nginx`, `https_granian`, and IPv6 dual-stack |
| `netbox_proxbox_mode` | `INPUT_NETBOX_PROXBOX_MODE` (workflow input) and the event type | `dev` for push/PR, `[dev, pypi]` on release |
| `netbox_version` | `.github/netbox-versions.json` | Certified NetBox tags: `v4.5.8`, `v4.5.9`, and `v4.6.0` through `v4.6.6` |
| `proxmox_service` | `scripts/ci_e2e_matrix.py` | PVE on every transport; PBS/PDM on every push/manual cell and the baseline release transport |

Push/manual runs use **7 × 1 × 9 × 3 = 189 cells**. Release runs use
**2 modes × 9 NetBox versions × (7 PVE transports + 1 baseline × 2 sidecar
services) = 162 cells**. The executable regression rejects every event matrix
above 256 jobs.
