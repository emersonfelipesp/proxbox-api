...
| `PROXBOX_AUTH_LOCKOUT_SOURCE_THRESHOLD` | `50` | Falhas permitidas entre todas as credenciais apresentadas por uma origem normalizada. Tambem limita os compromissos de credenciais distintas dessa origem. Intervalo 1-100000. |
...
| `PROXBOX_AUTH_LOCKOUT_MAX_BUCKETS` | `10000` | Maximo total de linhas duraveis de falha, dividido em particoes independentes de credencial/origem. Intervalo 2-1000000. Falhas sem chave alocam somente linhas de origem, e reservas ativas comprometem vagas futuras. Na saturacao, a linha expirada segura mais antiga e removida primeiro; uma via serializada globalmente e por origem continua disponivel para verificar uma credencial desconhecida sem permitir que identidades nao autenticadas esgotem toda admissao de chaves validas. |
...
| `PROXBOX_TRUSTED_PROXIES` | (vazio) | CIDRs/IPs de proxies reversos confiaveis, separados por virgula. Entradas invalidas interrompem o startup. Somente esses peers podem fornecer `X-Forwarded-For`; localhost nao possui bypass e confianca nunca ignora autenticacao ou bloqueio. Execute Uvicorn/FastAPI CLI com `--no-proxy-headers` para que essa politica veja primeiro o peer de transporte; os entrypoints distribuidos ja fazem isso. |
