...
| `PROXBOX_AUTH_LOCKOUT_MAX_BUCKETS` | `10000` | Máximo total de linhas duráveis de falha, dividido em partições independentes de credencial/origem. Intervalo 2-1000000. Reservas retidas comprometem uma vaga futura por identidade distinta. Na saturação, a linha expirada mais antiga sem reserva pendente é removida primeiro; se não houver vaga segura, identidades desconhecidas são negadas antes do bcrypt e contabilizadas como rejeições de capacidade. |
