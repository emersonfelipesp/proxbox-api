...
| `PROXBOX_RUNTIME_CODEGEN_ENABLED` | `false` | Opt-in de processo exclusivo para desenvolvimento. Quando falso, os endpoints HTTP de geracao e atualizacao nao existem, e o registro de rotas, a descoberta e a renderizacao Pydantic usam somente schemas incluidos. A producao deve manter essa opcao desabilitada, pois sidecars de proveniencia detectam corrupcao, mas nao autenticam arquivos gravaveis pelo mesmo usuario do sistema operacional. |
