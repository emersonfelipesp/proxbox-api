...
| `POST /proxmox/console/sessions` e `POST /proxmox/console/browser-sessions` | HTTP 403 antes de resolver endpoint, descriptografar, conectar ou criar proxy | Contratos privados e de navegador autônomo criptografado para QEMU noVNC, terminal QEMU e terminal LXC com cliente Proxmox sob responsabilidade explícita |
| `WS /proxmox/console/browser-stream` | Código 1008 antes de consumir estado de relay de uso único ou conectar ao upstream | Relay de navegador autônomo vinculado à Origin e criptografado com o contrato existente de token de uso único |
...
outro worker legado do relay já possui.
...
O control plane complementar usa seu próprio modo de execução fixado pelo
operador e a mesma `PROXBOX_EXECUTION_GENERATION`, mas responde por seu
consumidor de fila e relay.
...
Este componente não implementa revogação de tickets na fila complementar nem admissão da UI antes
