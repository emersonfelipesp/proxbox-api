...
- Credential-isolated lockouts use versioned `auth_lockout_buckets` plus
  ...
  capacity without deleting crash evidence, and durable failure rows use
...
| `PROXBOX_AUTH_LOCKOUT_SOURCE_THRESHOLD` | `50` | Failed attempts allowed across all presented credentials for one normalized source. It also caps distinct credential commitments by that source. Valid range: 1-100000. |
...
| `PROXBOX_AUTH_LOCKOUT_MAX_BUCKETS` | `10000` | Maximum total durable failure rows, divided into independent credential/source partitions. Valid range: 2-1000000. Missing-key failures allocate only source rows, and active reservations commit future slots. At saturation, the stalest safe expired row is evicted first; one globally serialized, per-source lane remains for verifying an unseen credential without letting unauthenticated identities exhaust all valid-key admission. |
...
| `PROXBOX_TRUSTED_PROXIES` | (empty) | Comma-separated CIDRs/IPs for trusted reverse proxies. Invalid entries fail startup. Only requests from these peers may use `X-Forwarded-For`; no localhost bypass exists, and trust never bypasses authentication or lockout. Run Uvicorn/FastAPI CLI with `--no-proxy-headers` so this application policy sees the transport peer first; shipped entrypoints already do so. |
