...
| `PROXBOX_AUTH_MAX_ACTIVE_KEYS` | `32` | Maximum active API-key hashes one request may examine. Valid range: 1-1024. If more active keys exist, authentication returns transient capacity pressure before bcrypt runs; raise the bound temporarily to retire excess keys. |
...
| `PROXBOX_TRUSTED_PROXIES` | empty for raw/Granian; `127.0.0.1/32` for bundled nginx | Comma-separated CIDRs/IPs for trusted reverse proxies. Invalid entries fail startup. Only requests from these peers may use `X-Forwarded-For`, and trust never bypasses authentication or lockout. The single-purpose nginx entrypoint always prepends its protected loopback hop. External reverse-proxy deployments must explicitly configure the exact proxy peer CIDRs and keep the application port private. Run Uvicorn/FastAPI CLI with `--no-proxy-headers`; shipped entrypoints already do so. |
