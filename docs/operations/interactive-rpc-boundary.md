...
| `POST /proxmox/console/sessions` and `POST /proxmox/console/browser-sessions` | Static HTTP 403 before endpoint resolution, decryption, connection, or proxy creation | Private and encrypted standalone-browser QEMU noVNC, QEMU terminal, and LXC terminal contracts with an owned Proxmox client |
| `WS /proxmox/console/browser-stream` | Close 1008 before one-use relay state consumption or upstream connection | Origin-bound, encrypted standalone browser relay with the existing one-use token contract |
...
process or a console token already held by an old relay worker.
...
The companion control plane uses its own operator-pinned execution mode and the
same `PROXBOX_EXECUTION_GENERATION`, but owns its queue consumer and relay
status independently. A coordinator must verify compatible published capabilities,
...
Missing companion-service support, unknown workers, unresolved remote effects, or
...
This component does not implement companion queue ticket revocation or UI
