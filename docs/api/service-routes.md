...
`GET /ceph/status` returns HTTP 404 with
`detail.reason=ceph_endpoint_not_configured` when no Proxmox endpoint is
configured. If a configured endpoint cannot establish a Proxmox session, the
route returns HTTP 502, or the upstream Proxmox HTTP error status when one is
available, with `detail.reason=proxmox_session_acquisition_failed`. The
`python_exception` field contains only the sanitized exception class name;
internal exception text remains hidden unless `PROXBOX_EXPOSE_INTERNAL_ERRORS`
