...
def test_netbox_api_from_endpoint_is_cached_by_config(monkeypatch):
    ...
    first = asyncio.run(netbox_session_module.netbox_api_from_endpoint(endpoint))
    second = asyncio.run(netbox_session_module.netbox_api_from_endpoint(endpoint))
...
def test_get_netbox_async_session_returns_async_facade(monkeypatch, db_engine):
    with Session(db_engine) as session:
        ...
        async def _fake_netbox_api_from_endpoint(ep, **_kwargs):
            return async_facade

        monkeypatch.setattr(
            netbox_session_module,
            "netbox_api_from_endpoint",
            _fake_netbox_api_from_endpoint,
        )
