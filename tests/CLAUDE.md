...
| `test_auth_lockout.py` | Composite credential isolation plus shared source-abuse limits, database-bound and process-pinned identity-key generations/loss/skew/post-start-mutation contracts, atomic key-file publish fault injection, independent per-token reservation expiry, exactly-once duplicate/late-finalizer recovery within the supported horizon, finalizer-driven orphan compaction, cross-worker failure-cohort coalescing, pending-identity partition commitments, per-bucket/global admission limits, typed WebSocket auth frames, sync/async/HTTP/WebSocket valid bursts above failure thresholds, durable source budgets/counters, fail-closed partition saturation plus safe expired-row eviction, trusted/untrusted proxy normalization, sync/async and multiprocess atomic races, busy-timeout-before-WAL sync/async contention, full serialized bootstrap, rollback-compatible legacy schema, strict bucket/reservation/metric/key-binding validation, and label-free capacity/row/in-flight/orphan/compaction metrics |
| `test_auth_lockout_cli.py` | Explicit-existing-database enforcement, complete recovery-schema validation, read-only secret-safe inspection/recovery while HTTP is locked, and offline runtime-lease-enforced identity-key rebind that atomically clears buckets/reservations and advances or safely recreates the generation |
