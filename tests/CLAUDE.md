...
| `test_auth_lockout.py` | Composite credential isolation plus shared source-abuse limits, database-bound and process-pinned identity-key generations/loss/skew/post-start-mutation contracts, independent per-token reservation expiry, exactly-once duplicate/late-finalizer recovery within the supported horizon, bounded orphan compaction, per-bucket/global admission limits, typed WebSocket auth frames, sync/async/HTTP/WebSocket valid bursts above failure thresholds, durable source budgets/counters, partition-saturated valid-key admission plus aggregate accounting, trusted/untrusted proxy normalization, sync/async and multiprocess atomic races, busy-timeout-before-WAL sync/async contention, full serialized bootstrap, rollback-compatible legacy schema, strict bucket/reservation/metric/key-binding validation, and label-free capacity/row/in-flight/orphan/compaction metrics |
