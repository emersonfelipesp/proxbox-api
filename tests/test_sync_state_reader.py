"""Tests for typed sidecar sync-state reads."""
...
@pytest.mark.asyncio
async def test_vm_identity_resolver_returns_none_when_sidecar_absent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
async def test_dispatch_create_does_not_create_when_sync_state_resolution_refused(
    ...
    prepared = sync_vm._PreparedVMState(
        ...
        vm_type="qemu",
        sync_state_fields={"proxmox_endpoint_id": 500, "proxmox_vm_id": 101},
    )
