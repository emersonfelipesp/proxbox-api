...
def test_runtime_codegen_process_opt_in_accepts_only_true_values(monkeypatch, value):
    ...
    assert runtime_settings.runtime_codegen_enabled() is True


@pytest.mark.parametrize("value", [None, "", "0", "false", "no", "off", "invalid"])
def test_runtime_codegen_process_opt_in_defaults_off(monkeypatch, value):
    if value is None:
        monkeypatch.delenv("PROXBOX_RUNTIME_CODEGEN_ENABLED", raising=False)
    else:
        monkeypatch.setenv("PROXBOX_RUNTIME_CODEGEN_ENABLED", value)

    assert runtime_settings.runtime_codegen_enabled() is False
