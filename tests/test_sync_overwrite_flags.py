...
Locks in the canonical 26-flag set, safe defaults, and the OpenAPI
...
def test_overwrite_flags_field_count() -> None:
    ...
    assert len(SyncOverwriteFlags.model_fields) == 26
...
def test_overwrite_flags_defaults_preserve_historical_ownership() -> None:
    """Existing overwrite gates stay enabled; VM platform ownership stays opt-in."""
    flags = SyncOverwriteFlags()
    for name in EXPECTED_FLAGS:
        expected = name != "overwrite_vm_platform"
        assert getattr(flags, name) is expected, f"unexpected default for {name}"
...
def test_overwrite_flags_individually_settable_to_false(flag_name: str) -> None:
    ...
    for other in EXPECTED_FLAGS:
        expected = other != flag_name and other != "overwrite_vm_platform"
