...
- `PROXBOX_AUTH_LOCKOUT_MAX_BUCKETS`: maximum durable failure rows (default 10000, validated range 2-1000000), split into independent credential/source partitions. Inactive rows older than the configured window are pruned; saturation never blocks bcrypt for a valid unseen key, and rejected identities that cannot persist are aggregate-counted.
