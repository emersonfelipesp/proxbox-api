...
- **Packer preflight + response privacy** (`POST /cloud/templates/images/preflight`): v1 resolves the exact enabled persisted `endpoint_id` to exactly one database-backed minimal session (never first-session fallback or broad post-connect discovery), then uses GET-only node/storage/VMID checks and remains available with `allow_writes=False`. VMID authority is `cluster/nextid?vmid=`; resource enumeration is supplemental. Malformed collections and absent storage health state fail closed as `unsupported`; normalized readiness requires ISO `image_storage` only for `proxmox_iso`, VM `images`, and conditional `snippets`, while `content=import` is only the separate `download-url` POST request value. Executable approvals include the rendered `recipe_digest` and receive a signed five-minute plan bound to current endpoint/target/storage/VMID/recipe state. Execution reruns preflight, consumes the plan once, acquires a durable unique target lease, runs asynchronously with bounded stream draining and cancellation, and records `completed` only after final API artifact verification; partial state is preserved as `recovery_required`. Findings are fixed `{code,severity,target,message}` objects and session/upstream exceptions never reach responses or logs. `POST /cloud/templates/images` returns secret-safe response v2: URLs, cloud-init, scripts, commands, stdout, and stderr are omitted by default and during execution; unexpected execution/direct-SDK errors are normalized to fixed diagnostics and type-only logs. Raw material is available only under the explicitly sensitive preview with `execute=false` plus `include_sensitive_preview=true`. Preflight v1 and build response v2 are supported through `0.0.21.x`; `storage` remains an input-only alias of `vm_storage` during that window, and a breaking replacement is no earlier than `0.0.22.0`. See `routes/cloud/CLAUDE.md` and `docs/architecture/overview.md` for lifecycle evidence.
