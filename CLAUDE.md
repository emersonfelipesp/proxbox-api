...
- `PROXBOX_AUTH_LOCKOUT_MAX_BUCKETS`: maximum durable failure rows (default 10000, validated range 2-1000000), split into independent credential/source partitions. Missing-key failures allocate only source rows, active reservations commit future slots, and a source's distinct commitments are bounded by its source threshold. At saturation, the stalest safe expired row is evicted first and one globally serialized, per-source verification lane preserves valid-key admission.
...
- `PROXBOX_TRUSTED_PROXIES`: comma-separated explicit proxy CIDRs allowed to supply `X-Forwarded-For`. Empty trusts no caller; invalid entries fail startup; localhost has no implicit exemption. Uvicorn/FastAPI entrypoints must use `--no-proxy-headers` so the application resolves trust from the real transport peer.
