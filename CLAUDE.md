...
- `PROXBOX_AUTH_LOCKOUT_MAX_BUCKETS`: maximum durable failure rows (default 10000, validated range 2-1000000), split into independent credential/source partitions. Retained reservations commit one future slot per distinct identity. At saturation, the stalest expired unreserved row is evicted first; if no safe slot exists, unseen identities are denied before bcrypt and counted as capacity rejections.
...
- `PROXBOX_TRUSTED_PROXIES`: comma-separated explicit proxy CIDRs allowed to supply `X-Forwarded-For`. Empty trusts no caller; invalid entries fail startup; localhost has no implicit exemption.
