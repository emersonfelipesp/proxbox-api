...
- **Extras custom fields** (`routes/extras/`, `/extras/*`): `POST /extras/custom-fields/reconcile` forces a live NetBox custom-field reconcile from the canonical inventory in `services/custom_fields.py`, bypassing the process-local cache; `GET /extras/bootstrap-status` exposes startup bootstrap warnings; legacy `GET /extras/extras/custom-fields/create` remains for backward compatibility.
