...
- `PROXBOX_AUTH_MAX_ACTIVE_KEYS`: maximum active API-key hashes examined by one request (default 32, validated range 1-1024). If the database exceeds the configured bound, authentication returns transient capacity pressure before bcrypt; temporarily raise the bound to retire excess keys.
...
- `PROXBOX_TRUSTED_PROXIES`: comma-separated explicit proxy CIDRs allowed to supply `X-Forwarded-For`. Empty trusts no caller in raw/Granian deployments; invalid entries fail startup. The bundled single-purpose nginx entrypoint always prepends its protected `127.0.0.1/32` Uvicorn hop. External proxy deployments must list exact proxy peer CIDRs and keep the application port private. Uvicorn/FastAPI entrypoints must use `--no-proxy-headers` so the application resolves trust from the real transport peer.
