...
- `PROXBOX_AUTH_LOCKOUT_MAX_BUCKETS`: maximum durable credential/source rows (default 10000, validated range 2-1000000); only inactive, non-reserved rows are eligible for eviction, and admission fails closed when none are available.
...
- `PROXBOX_AUTH_LOCKOUT_HMAC_KEY_FILE`: optional identity-key path; defaults beside the SQLite database as `<database>.auth-lockout.key`. It must share durable storage with the database and have no group/other permissions.
