...
from proxmox_mock.codegen.pydantic_generator import (
    generate_pydantic_models_from_openapi,
)
from proxmox_mock.codegen.utils import extract_path_params, pascal_case, slugify_identifier
from proxmox_mock.errors import ProxmoxMockError
from proxmox_mock.openapi import DEFAULT_PROXMOX_OPENAPI_TAG, load_packaged_openapi
...
from proxmox_mock.state import shared_mock_store
...
def _load_model_module(openapi_document: dict[str, object], version_tag: str) -> ModuleType:
    ...
    module = ModuleType(f"proxmox_mock.generated_models_{version_tag.replace('.', '_')}")
...
def _build_endpoint_signature(
    *,
    ...
    request_schema: dict[str, Any] | None,
) -> tuple[list[inspect.Parameter], dict[str, str], dict[str, str]]:
    ...
    return signature_parameters, path_param_map, query_param_map
...
def _build_generated_endpoint(
    *,
    ...
    signature_parameters, path_param_map, query_param_map = _build_endpoint_signature(
        topology=topology,
...
def register_generated_proxmox_mock_routes(
    ...
    *,
    ...
    document = openapi_document or load_packaged_openapi(version_tag=version_tag)
    if not document:
        raise ProxmoxMockError(
