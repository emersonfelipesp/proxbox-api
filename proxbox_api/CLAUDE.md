...
- `routes/proxmox/console.py` preserves the private service-only session broker and owns the standalone browser create/WebSocket boundaries. Reusable state, encryption, RFB 3.8 mediation, and bidirectional relay logic belong in `services/console_relay.py`; the inactive future RPC-only check belongs only in `services/console_relay_policy.py` and is called at create and consume.
