...
This file lives at `<repository-root>/proxbox_api/ceph/CLAUDE.md` inside the `personal-context` workspace.
...
| File | Role |
|------|------|
| `routes.py` | FastAPI router for `/ceph/status` and `/ceph/sync/{status,daemons,osds,pools,filesystems,crush,flags,rgw,rbd,full}`. `/ceph/status` returns a typed 404 when no Proxmox endpoint is configured and preserves sanitized 502/upstream session-acquisition failures. RGW/RBD routes preserve the `CephSyncResponse` summary envelope and include reflected inventory in `raw`. |
