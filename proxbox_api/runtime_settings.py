...
def runtime_codegen_enabled() -> bool:
    """Return the process-level runtime codegen opt-in.

    The application factory needs this value before a NetBox connection exists,
    so it intentionally has no plugin-settings fallback.
    """

    return os.environ.get("PROXBOX_RUNTIME_CODEGEN_ENABLED", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
__all__ = ["get_int", "get_float", "get_bool", "get_str", "runtime_codegen_enabled"]
