...
class ReconcileResult:
    """Outcome of a drift-detecting reconcile call.

    ``status`` reports whether the call resulted in a POST (``"created"``),
    a PATCH (``"updated"``), or a no-op (``"unchanged"``) against NetBox.
    """
...
async def _rest_reconcile_async_impl(  # noqa: C901
    nb: object,
    path: str,
    *,
    lookup: dict[str, object],
    ...
    Public callers should use :func:`rest_reconcile_async` (record only,
    back-compat) or :func:`rest_reconcile_async_with_status` (record + status).
    """
    api = _unwrap_api(nb)
    normalized_path = _normalize_path(path)
...
async def rest_reconcile_async(
    ...
    *,
    ...
    Returns only the resulting record. Call
    :func:`rest_reconcile_async_with_status` when the caller needs to know
    whether the operation was a create, update, or no-op.

    Args:
        strict_lookup: When True, only the exact ``lookup`` dict is used to find
            an existing record.  The fallback candidates generated by
            ``_candidate_reuse_lookups`` (e.g. name-only) are skipped.  Use this
...
async def rest_reconcile_async_with_status(
    nb: object,
    path: str,
    *,
    lookup: dict[str, object],
    ...
    Drift-detecting wrapper around :func:`rest_reconcile_async` that exposes the
    operation outcome so callers can surface per-resource counters in SSE
    summaries and assert idempotency in tests.
    """
    record, status = await _rest_reconcile_async_impl(
        nb,
        path,
        lookup=lookup,
        payload=payload,
