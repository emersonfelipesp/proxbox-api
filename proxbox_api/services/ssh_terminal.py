...
async def _connect_terminal(credential: TerminalCredential) -> SSHClientConnection:
...
async def _close_terminal_connection(conn: SSHClientConnection) -> None:
    conn.close()
    await conn.wait_closed()


async def _terminal_process(conn: SSHClientConnection, session: TerminalSession) -> object:
    require_interactive()
...
def _verify_terminal_host_key(conn: SSHClientConnection, credential: TerminalCredential) -> None:
    """Verify the acquired transport before PTY even if an SDK skips its callback."""
    require_interactive()
    key = conn.get_server_host_key()
    actual = _fingerprint_from_key(key)
