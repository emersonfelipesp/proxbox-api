...
- Update and delete handlers await targeted runtime-client invalidation after the database commit and before returning, so rotated credentials cannot leave a retired transport open. Update also rebuilds and republishes the default lifecycle-owned client before the response, preserving raw/WebSocket consumers.
...
- Preserve awaited lifecycle invalidation when changing endpoint mutation routes.
