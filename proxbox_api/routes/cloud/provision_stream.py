...
async def _provision_stream_generator(
    ...
    try:
        yield sse_event("provision_step", {
            "step": "open_session",
        ...
        try:
            ...
            yield sse_event("complete", {"ok": False, "error": safe, "step": "configure_cloud_init"})
            return
        ...
        yield sse_event("terminal_line", {"line": f"✓ VM {req.new_vmid} provisioned · status={vm_status}"})
