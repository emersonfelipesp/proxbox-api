"""UPDATE dispatcher for LXC intent diffs."""
...
LXC_OFFLINE_REQUIRED_KEYS = {"cores", "memory"}
LXC_OFFLINE_REQUIRED_PREFIXES = ("mp", "rootfs")


def _build_lxc_update_delta(
    ...
    if "hostname" in fields_set and payload.hostname is not None:
        set_delta_if_changed(delta, current, "hostname", payload.hostname)
...
def _requires_vm_stop(delta: dict[str, object]) -> bool:
    for key in delta:
        if key in LXC_OFFLINE_REQUIRED_KEYS or key.startswith(LXC_OFFLINE_REQUIRED_PREFIXES):
            return True
    return False
...
async def dispatch_lxc_update(
    ...
    *,
    ...
    try:
        logger.debug(
            "intent.apply: lxc update dispatching vmid=%s payload=%s",
            payload.vmid,
            safe_payload,
        )
        proxmox = await _open_proxmox_session(gated)

        node_containers = sequence_from_response(
            await resolve_async(proxmox.session.nodes(payload.node).lxc.get())
        )
        if find_vmid_record(node_containers, vmid=payload.vmid, kind="lxc") is None:
            cluster_vms = sequence_from_response(
                await resolve_async(proxmox.session("cluster/resources").get(type="vm"))
            )
            current_node = find_current_node(cluster_vms, vmid=payload.vmid, kind="lxc")
            message = (
        ...
        current_config = mapping_from_response(
            await resolve_async(proxmox.session.nodes(payload.node).lxc(payload.vmid).config.get())
        )
        delta = _build_lxc_update_delta(payload, current_config)
        if not delta:
            await write_intent_journal(
                journal_writer=write_verb_journal_entry,
                endpoint_context=endpoint_context,
            ...
            return ApplyResultItem(
                ...
                kind="lxc",
                status="skipped",
                message="No LXC update needed; current Proxmox config already matches payload",
            )

        current_status = await resolve_async(
            proxmox.session.nodes(payload.node).lxc(payload.vmid).status.current.get()
        )
        ...
        response = await resolve_async(
            proxmox.session.nodes(payload.node).lxc(payload.vmid).config.put(**delta)
        )
        upid = extract_upid(response)
    except Exception as error:  # noqa: BLE001
        message = scrub_message(str(error), raw_payload)
        logger.warning(
            "intent.apply: lxc update failed vmid=%s node=%s error=%s",
            payload.vmid,
            payload.node,
            message,
        )
    ...
    return ApplyResultItem(
        ...
        status="succeeded",
        proxmox_upid=upid,
        message=f"Updated LXC {payload.vmid}: {', '.join(sorted(delta))}",
    )
