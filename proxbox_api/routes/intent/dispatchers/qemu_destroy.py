"""Destroy dispatcher for QEMU deletion requests."""
...
async def dispatch_qemu_destroy(endpoint, vmid: int, node: str, run_uuid, *, actor) -> dict:
    ...
    try:
        logger.info(
            "intent.deletion_requests: qemu destroy dispatching vmid=%s node=%s", vmid, node
        )
        proxmox = await _open_proxmox_session(gated)
        response = await resolve_async(
            proxmox.session.delete("nodes", node, "qemu", str(vmid), purge=1)
        )
        upid = extract_upid(response)
    except Exception as error:  # noqa: BLE001
        message = scrub_message(str(error))
        logger.warning(
            "intent.deletion_requests: qemu destroy failed vmid=%s node=%s error=%s",
            vmid,
            node,
            message,
        )
